...
def openai_generate_user_reply(user_text: str) -> str:
    ...
    user_prompt = (
        "งานของคุณ:\n"
        "1) เดาว่าข้อความผู้ใช้น่าจะเข้ากลุ่มไหนในรายการ (หรือ none)\n"
        "2) แล้วตอบกลับให้เหมาะกับความหมายและสถานการณ์\n\n"
        f"รายชื่อกลุ่มที่มี: {ALLOWED_GROUPS} และ none\n\n"
        "ตัวอย่างโทน/สไตล์ (ห้ามคัดลอกตรงๆ):\n"
        f"{ref_block}\n\n"
    )
    if avoid_block:
        user_prompt += (
            "ประโยคที่เพิ่งตอบไปไม่นาน (พยายามอย่าให้ซ้ำโครง/คำเด่นๆ):\n"
            f"{avoid_block}\n\n"
        )
    ...
    user_prompt += (
        ...
        "กติกา:\n"
        "- ตอบเป็นภาษาไทยล้วน\n"
        "- สั้นเป็นหลัก (1-2 ประโยค) แต่ถ้าผู้ใช้ถามจริงจังให้ตอบให้มีประโยชน์\n"
        "- แต่งใหม่ทุกครั้ง\n\n"
        "ตอบกลับ:"
    )
...
def openai_generate_scheduled(schedule_id: str, meaning: str, examples: List[str]) -> str:
    ...
    prompt = (
        "สร้างข้อความสำหรับส่งตามเวลา 1 ข้อความ\n"
        f"ความหมาย/จุดประสงค์: {meaning}\n"
        "ตัวอย่างโทน/สไตล์ (ห้ามคัดลอกตรงๆ):\n"
        f"{ex}\n\n"
        "กติกา:\n"
        "- ภาษาไทยล้วน\n"
        "- สั้นถึงกลาง (อย่ายาวเป็นย่อหน้า)\n"
        "- แต่งใหม่ทุกครั้ง\n"
    )
    if avoid:
        prompt += (
